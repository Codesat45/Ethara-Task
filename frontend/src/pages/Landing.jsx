...
/* ═══════════════════════════════════════════════════════════════
   VIDEO SOURCES — 4 free Pexels videos, each tied to a section
   1. Hero    → warehouse workers / logistics operations
   2. Features → data center / server room (tech infrastructure)
   3. Process  → business team / office workflow
   4. CTA      → aerial city / supply chain overview
═══════════════════════════════════════════════════════════════ */
const VIDEOS = {
...
/* ─────────────────────────────────────────────────────────────
   SectionVideo — reusable full-bleed video background component
   Accepts: src, overlayColor, overlayOpacity, children
   Gracefully degrades to a gradient if video fails.
───────────────────────────────────────────────────────────── */
function SectionVideo({ src, tint = 'rgba(4,4,4,0.78)', accent = 'rgba(229,9,20,0.05)', children, className = '' }) {
  ...
  return (
    <section className={`relative overflow-hidden ${className}`}>
      ...
      {err && (
        <div className="absolute inset-0 bg-gradient-to-br from-[#0a0a0a] via-[#111] to-[#080808]" style={{ zIndex: 0 }} />
      )}

      {/* ── Overlay stack (HackerRank style) ── */}
      ...
      <div className="absolute inset-0 pointer-events-none" style={{
        background: `radial-gradient(ellipse 80% 60% at 50% 50%, ${accent} 0%, transparent 70%)`,
        zIndex: 2,
      }} />
      {/* Layer 3: top + bottom edge fades */}
      <div className="absolute inset-0 pointer-events-none" style={{
        background: 'linear-gradient(to bottom, rgba(8,8,8,0.55) 0%, transparent 18%, transparent 82%, rgba(8,8,8,0.85) 100%)',
        zIndex: 3,
      }} />
...
function Typewriter({ text, speed = 60 }) {
  ...
  return (
    <span>
      {d}
      <span className="inline-block w-0.5 h-[0.9em] bg-[#E50914] ml-0.5 align-middle"
        style={{ animation: d.length < text.length ? 'none' : 'blink 1s step-end infinite' }} />
    </span>
  )
}

/* ─────────────────────────────────────────────────────────────
   Feature card — glass morphism style for video backgrounds
───────────────────────────────────────────────────────────── */
...
export default function Landing() {
  ...
  return (
    <div className="bg-[#080808] text-white overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 transition-all duration-500"
        ...
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2"
            ...
            <Link to="/login" className="text-white/60 hover:text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-white/5 transition-all duration-200">
              Sign In
            </Link>
      ...
      {/* ══════════════════════════════════════════════════════
          SECTION 1 — HERO  (Video: warehouse / logistics)
      ══════════════════════════════════════════════════════ */}
      <SectionVideo
        src={VIDEOS.hero}
        tint="rgba(3,3,3,0.75)"
        accent="rgba(229,9,20,0.07)"
      ...
      >
        <div className="text-center px-6 max-w-4xl mx-auto py-32">
          ...
          <div className="inline-flex items-center gap-2.5 rounded-full px-4 py-1.5 mb-10 text-xs font-bold uppercase tracking-widest"
            style={{
              background: 'rgba(229,9,20,0.1)', border: '1px solid rgba(229,9,20,0.25)', color: '#E50914',
              opacity: heroLoaded ? 1 : 0, transform: heroLoaded ? 'none' : 'translateY(-12px)',
          ...
          <h1 className="font-black leading-[1.02] tracking-tight mb-6"
            ...
            Control Your<br />
            <span style={{ color: '#E50914', textShadow: '0 0 40px rgba(229,9,20,0.5), 0 2px 40px rgba(0,0,0,0.8)' }}>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4"
            ...
            <Link to="/login" className="btn-outline text-base py-3.5 px-10" style={{ backdropFilter: 'blur(8px)', background: 'rgba(255,255,255,0.05)' }}>
              Sign In
            </Link>
          </div>
      ...
      ══════════════════════════════════════════════════════ */}
      <div className="relative z-10 bg-[#080808] border-y border-white/5 py-10 px-6">
      ...
      {/* ══════════════════════════════════════════════════════
          SECTION 2 — FEATURES  (Video: data center / servers)
      ══════════════════════════════════════════════════════ */}
      <SectionVideo
        src={VIDEOS.features}
        tint="rgba(2,2,8,0.82)"
        ...
        <div className="max-w-5xl mx-auto">
          <Reveal className="text-center mb-16">
            ...
            <p className="text-white/50 mt-4 text-lg max-w-xl mx-auto">
              A complete inventory system — from product catalog to order fulfillment.
            </p>
          </Reveal>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <FeatureCard delay={0} accent="#E50914"
            ...
            />
            <FeatureCard delay={80} accent="#10b981"
            ...
            />
            <FeatureCard delay={160} accent="#a855f7"
            ...
            />
            <FeatureCard delay={240} accent="#3b82f6"
          ...
          <Reveal delay={300} className="flex flex-wrap justify-center gap-3 mt-14">
            {['FastAPI', 'React 18', 'PostgreSQL', 'JWT Auth', 'Tailwind CSS', 'Docker', 'Render'].map(t => (
              <span key={t} className="text-xs font-semibold px-4 py-2 rounded-full"
      ...
      {/* ══════════════════════════════════════════════════════
          SECTION 3 — HOW IT WORKS  (Video: office / workflow)
      ══════════════════════════════════════════════════════ */}
      <SectionVideo
        src={VIDEOS.process}
        tint="rgba(4,4,4,0.80)"
      ...
      >
        <div className="max-w-5xl mx-auto">
          <Reveal className="text-center mb-16">
            ...
            <p className="text-white/50 mt-4 text-lg max-w-xl mx-auto">
              Three simple steps to take full control of your inventory.
            </p>
          </Reveal>
              ...
              <Reveal key={step.n} delay={i * 140} className="text-center">
                <div className="relative inline-flex items-center justify-center mb-6">
                  <div className="w-20 h-20 rounded-full absolute"
                    style={{ border: `1px solid ${step.accent}30`, animation: `spin-slow ${14 + i * 3}s linear infinite` }} />
            ...
            <div className="rounded-2xl p-6 md:p-8 flex flex-col md:flex-row items-center gap-6 text-center md:text-left"
              ...
              <div className="flex-1">
                ...
                <p className="text-white/45 text-sm leading-relaxed">
                  Every endpoint is protected by 256-bit JWT tokens. Passwords are bcrypt-hashed. Stock validation runs server-side — no client-side tricks.
                </p>
              </div>
      ...
      {/* ══════════════════════════════════════════════════════
          SECTION 4 — CTA  (Video: aerial city / supply chain)
      ══════════════════════════════════════════════════════ */}
      <SectionVideo
        src={VIDEOS.cta}
        tint="rgba(3,3,3,0.78)"
        accent="rgba(229,9,20,0.08)"
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="border-t border-white/5 pt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
            <p className="text-white/20 text-xs">
              © {new Date().getFullYear()} StockFlow — Inventory & Order Management System
            </p>
